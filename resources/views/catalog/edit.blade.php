...
@section('product', 'Редактирование товара')

@section('content')
    <style>
        /* ===== СТИЛИ АДМИНСКОЙ ЧАСТИ (СИНЯЯ СХЕМА) ===== */
    ...
    <form action="{{ route('products.update', $data->id) }}" method="POST" enctype="multipart/form-data" class="edit-row">
        @csrf
        @method('PUT')
        ...
        <div class="edit-image-column">
            ...
            @php
                $imgSrc = asset($data->image);
            @endphp

            @if($imgSrc)
                <img src="{{ asset($imgSrc) }}" alt="{{ $product->name ?? 'Товар' }}" class="current-img">
            @else
        ...
        <div class="edit-form-column">
            <h2>Редактировать товар: <span><strong>{{ $data->name }}</strong></span></h2>
            ...
            <div class="form-group">
                ...
                <input type="number" step="0.01" name="price" id="price" value="{{ old('price', $data->price) }}" required
                       placeholder="0.00">
            </div>
            ...
            <div class="form-group">
                <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 15px;">
                    <div>
                        ...
                        <input type="number" step="1" name="height" id="height" value="{{ old('height', $data->height) }}" required
                               placeholder="100">
                    </div>
                    <div>
                        <label for="width">Ширина (см)</label>
                        <input type="number" step="1" name="width" id="width" value="{{ old('width', $data->width) }}" required
                               placeholder="50">
                    </div>
                </div>
            </div>
            ...
            <div class="form-group">
                <label for="light_requirement">Требования к освещению</label>
                <input type="text" name="light_requirement" id="light_requirement" value="{{ old('light_requirement', $data->light_requirement) }}"
                       placeholder="Например: яркий рассеянный свет" required>
            </div>
            ...
            <div class="form-group">
                ...
                <select name="category_id" id="category_id" required>
                    @foreach($categories as $cat)
                        <option value="{{ $cat->id }}" {{ $cat->id == old('category_id', $data->category_id) ? 'selected' : '' }}>
                            {{ $cat->name }}
                        </option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <textarea name="description" id="description" rows="5" required
                          placeholder="Опишите товар, его особенности и характеристики">{{ old('description', $data->description) }}</textarea>
            </div>
